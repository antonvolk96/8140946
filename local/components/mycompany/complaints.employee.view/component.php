...
global $USER;
###
\Bitrix\Main\Loader::includeModule('mycompany.complaints');
use MyCompany\COMPLAINTS;
...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"] === "N"
 ...
 'VALUE' => '01.' . $i . '.' . date('Y')
    ...
    $arResult['ITEMS'] =
        COMPLAINTS\ComplaintsManager::GetComplaintsEmployee($arParams["USER_ID"]);


    foreach($arResult['ITEMS'] as &$arComplaints) {
        $arValue = COMPLAINTS\ComplaintsManager::GetComplaintsEmployeeValue($arComplaints["ID"], $arParams["USER_ID"], $_REQUEST['UF_PERIOD']);
        $arComplaints["COMPLAINTS_ID"] = $arValue["ID"];
        $arComplaints["COMPLAINTS_ID_VALUE"] = $arValue["UF_VALUE"];
    }
